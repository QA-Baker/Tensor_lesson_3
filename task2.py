...
def get_list_info(lst):
    min_elem = min(lst)  # Минимальный элемент списка
    max_elem = max(lst)  # Максимальный элемент списка
    sum_list = sum(lst)  # Сумма всех элементов списка
    average = round(sum_list / len(lst), 2)  # Среднее арифметическое с округлением до 2 знаков
